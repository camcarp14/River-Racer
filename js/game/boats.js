...
(function () {
  ...
  const builders = {
    ...
    podracer(spec) {
      ...
      const silver = () => mat(0xd8dce0, { roughness: 0.35, metalness: 0.5 });   // turbine housings
      const brightChrome = () => mat(0xe9edf0, { roughness: 0.22, metalness: 0.6 });
      const tubeMat = mat(spec.hull, { roughness: 0.42, metalness: 0.55 });      // golden engine body + cowls
      const blueMat = mat(0x5a63b8, { roughness: 0.55, metalness: 0.2 });        // blue-lavender X marks + accents
      ...
      // twice the film detail without a draw-call explosion. Coordinates are LOCAL to an
      // engine axis; the s*EX/EY/EZ offset bakes in at push time.
      const silverGeos = [], goldGeos = [], darkGeos = [], chromeGeos = [], accentGeos = [];
      ...
      function engine(s) {
        ...
        for (let i = 0; i < 6; i++) {                                    // vertical machinery ribs
          const rib = new THREE.BoxGeometry(0.06, 0.2, 1.55);
          rib.translate(0, 0.88, 2.3); rib.rotateZ(Math.PI / 6 + i * Math.PI / 3);
          pushG(silverGeos, rib, s);
        }
        for (const zz of [3.08, 1.52]) {                                 // chrome end rings
          const ring = new THREE.TorusGeometry(0.84, 0.05, 6, 22); ring.translate(0, 0, zz);
          pushG(chromeGeos, ring, s);
        }
        const panel = new THREE.BoxGeometry(0.55, 0.06, 1.1); panel.translate(0, 0.86, 2.3);
        pushG(goldGeos, panel, s);                                       // gold trapezoid panel on the crown
        for (const d of [-1, 1]) {                                       // small lavender accents, upper flanks
        ...
        for (const d of [-1, 1]) {
          const pipe = cylZ(0.06, 0.06, 1.0, 6); pipe.translate(d * 0.6, 0.2, 1.05);
          pushG(chromeGeos, pipe, s);
        }
        pushG(goldGeos, cylZ(0.66, 0.5, 2.2, 16).translate(0, 0, -0.15), s);
      ...
      // merge + mount the static engine geometry (5 draw calls for both engines)
      for (const [geos, m] of [[silverGeos, silver()], [goldGeos, tubeMat], [darkGeos, darkM], [chromeGeos, brightChrome()], [accentGeos, blueMat]]) {
