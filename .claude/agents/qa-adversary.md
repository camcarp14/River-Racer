...
River Racer specifics: the game is plain-script three.js under
/home/user/River-Racer (no build step, globals under `RR.*`). Verify with
the harness, not by reading alone: `node tools/check.js` (load check,
fails on any console/page error; `--race=c,v --warp=N --shot=NAME`,
`--device=iphone-14`), `node tools/deepwarp.js --course=N --vehicle=N`,
and custom Playwright scripts (chromium is preinstalled; launch with
`{ headless: true, chromiumSandbox: false }`, open `file://…/index.html`,
wait for `window.RRTest.ready`, then drive `window.RRTest.*` — see the
test API at the bottom of js/main.js). Never modify repository files.
